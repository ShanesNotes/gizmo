...
- `scripts/simulation.gd`: `Pickup` data agent; `Enemy.xp_value`; combat consts
  ...
  1169-1185), xp pickup (949/1038).
- Review hardening (0009 review): a per-tick `last_events: Array[Dictionary]`
  (attack/hit/defeat/pickup/levelup), cleared each `tick` — the `GameEvent[]` seam
...
- `tests/run_simulation_tests.gd`: +9 combat checks (autofire damage, range+radius,
  death+removal, Spark drop value, collect→xp, level-up) and +11 hardening checks
  (corpse-skip ×2, nearest-of-two ×2, pickup cap ×2, events ×5) → **63 total**.
