# GZ-101 — E1 (audio-canon lab): convert remaining 11 cues

intent: Complete the sonic arc: the 7 path_a cues and 4 ambient cues not covered by GZ-031, converted per the same promoted law.
...
executable success criteria: lab `python3 validators/validate_all.py --root .` exits 0; game `${GODOT_BIN:-godot} --headless --path godot --import` exits 0 and `tools/godot/run_all_checks.sh` exits 0 with 16 OGGs + sidecars present in godot/audio/.
