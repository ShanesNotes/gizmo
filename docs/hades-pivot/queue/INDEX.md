...
| HZ-111 | campfire cinematic seam READY: call `play_campfire_sit()` (+ `resume_from_cinematic()`) on `gizmo_animation_controller.gd` to seat Gizmo with the looping `campfire_sit` pose (holds through the gameplay state map). LORE LANE: opening.tscn's Gizmo is the raw gizmo.glb (no clips/controller) — attach an `AnimationController` (this script) to it, set `skeleton_path`, then call the seam. Weapon mount stays visible; hide it for the shot. Proof: ceremony/assets/2026-07-07-gizmo-campfire_sit-inengine.png | night/assets → lore | assets | handoff-ready | — |
...
| HZ-LORE-3 | codex event wiring: emit first_* events into scripts/codex/codex_log from run lifecycle (system shipped PR37; app_shell/orchestrator event seam outside lore fence except AUDIO region) | core | lore | queued | — |
| HZ-LORE-4 | saints Voice-Design pass: replace premade-library casting with dedicated designed voices per canon register (batch provenance notes the interim casting) | lore | lore | queued | — |
| HZ-CORE-N1 | register 5 new SFX cue ids in the audio manifest: cast_lodge, cast_reclaim, elite_shield_break, pattern_quarantine_seal, pattern_quarantine_lift (gameplay already notifies them — PRs #40/#45) | lore/audio | core | queued | — |
| HZ-CORE-N2 | UI flare consumption: BoonDraft.offer_flare(best_rarity) + bargain_offered(boon) signals shipped (PR #43) — draft screen should flare epic/legendary offers and frame the bargain card with its cost_lines | design | core | queued | — |
| HZ-CORE-N3 | Mirror UI: surface spark-shard purchases (MetaState.purchase_grade_with_shards, prices 100/200, spendable_spark_shards) + Keeper Rank display from run-summary keys keeper_rank/keeper_rank_title/keeper_shards_to_next_rank (PR #40) | design | core | queued | — |
| HZ-CORE-N4 | Keeper Rank titles are placeholder canon (Cold Chassis → Keeper of the First Spark, scripts/meta/keeper_rank.gd) — lore lab to bless or recast | lore | core | queued | — |
| HZ-CORE-N5 | quarantine boundary visual polish: current read is telegraph markers + distance ticks; a lit wall/plane treatment worth a look-lane pass (boss_brain quarantine segments are the data source) | design/look | core | backlog | — |
| HZ-CORE-N6 | lantern_staff_01 kit mechanics (HZ-110 handoff): moveset + keepsake hooks against the shipped wrapper | core | assets→core | queued | — |
